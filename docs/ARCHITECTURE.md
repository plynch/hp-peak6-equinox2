...
1. Build event clusters keyed by canonical event identity.
2. Build proposition clusters inside each event cluster.
...
Router scoring uses normalized quote/depth fields from proposition clusters and does not branch on venue names or venue-native payload shapes.
